...
import { Layer, Brush } from '@/components/pojo/Enums';
...
export default class Networks extends Vue {
        ...
        updated() {
          // activate zoom
          if (this.enableBrush) { this.activateZoom(); } else {
            this.boundary = null;
          }
          // activate multi select
          if (this.enableMultiSelect) { this.activateMultiSelect(); }
        ...
        get networks(): Network[] {
          const _ = this; let path: any; const scale = this.currentScale / 1.5; let
            counter = 0;
        ...
        supportingLinks(edge: Edge, highlight: boolean = true) {
          ...
          const [sid, tid, layer] = [edge.source!.id, edge.target!.id, edge.layer]; let selector: string = '';
          ...
          if (selector) { d3.selectAll(selector).classed('highlight', highlight).attr('stroke-width', sw).attr('stroke-dasharray', sd); }
        }
        ...
        private relayoutNetwork(net: Network, x: number, y: number) {
          ...
          net.center = [net.center[0] + x, net.center[1] + y];
            ...
            if (d3.event.sourceEvent) { d3.event.sourceEvent.stopImmediatePropagation(); }
        }
            ...
            if (id) {
              ...
              if (net.multiSelected) {
                ...
                _.selectedNetworks.map((id: string, i: number) => {
                  ...
                  if (n.multiSelected) { _.relayoutNetwork(n, x, y); }
                });
              } else {
            ...
            network.data!.map((d: Device) => {
              d.visible = false;
                d.links!.map((id: string) => {
                  const link = _.entrySet[id]; let isValidSrc; let isValidTarget; let src; let
                    dest;
          ...
          if (!network.expanded) {
            ...
            _.expandedNetworks.push(network.id);
                network.data!.map((d: Device) => {
                  d.visible = true;
                    d.links!.map((id: string) => {
                      const link = _.entrySet[id]; let src; let dest; let isValidSrc; let
                        isValidTarget;
                      if (link && link.source && link.target) {
                        src = link.source;
                        dest = link.target;
        ...
        private activateZoom() {
          ...
          const _ = this; const { width } = this.svg; const { height } = this.svg; let currMapScale = this.currentScale;
        ...
        private registerBrush() {
          ...
          d3.select('body').on('keydown', () => {
            ...
            if (d3.event.ctrlKey) {
              ...
              if (_.enableBrush) { _.enableBrush = false; } else if (_.selectedNetworks.length > 0) { _.cleanSelectedNetworks(); } else if (_.selectedDevices) {
        ...
        private refreshActiveDevice(d: Device, clean: boolean = true) {
          ...
          d.active = !d.active;
          if (d.active) { this.selectedDevices.push(d.id); } else { this.selectedDevices = this.selectedDevices.filter((id: string) => id != d.id); }
        }
        ...
        private zoomIn(d: any) {
          let scale: any; let translate: any; const _ = this; const width = _.svg.width || 960; const
        ...
        private dragDevice(id: string, dx: number, dy: number) {
          ...
          if (this.selectedDevices.length > 0 && this.selectedDevices.indexOf(id) != -1) {
            this.selectedDevices.map((key: string) => {
              d = this.entrySet[key];
              if (d) { this.relayoutDevice(d3.select(`#${key}`), d, dx, dy); }
            });
          } else {
        ...
        private linkPathCalcFn(e: Edge) {
          let diffX: number; let diffY: number; let arc; let x1; let y1; let x2; let y2; let
            path;
        ...
        private networkPath(n: Network) {
          const _ = this; const points: any[] = []; let hullPoints: any; let centerid: any; let
            toppoint: any;
            n.data!.map((d: Device) => {
              if ((_.projection(d.point))) { points.push([d.cx || _.projection(d.point)![0], d.cy || _.projection(d.point)![1]]); }
            });
            if (points.length < 1) {
            ...
            if (centerid && isNaN(centerid[0])) { centerid = hullPoints[0]; }
            return {
              center: centerid,
        ...
        private clickIcon(type: string) {
          ...
          switch (type) {
            case 'zoom':
              if (this.enableMultiSelect) { this.enableMultiSelect = false; }
              this.enableBrush = !this.enableBrush;
              break;
            case 'select':
              if (!this.enableSelect) { return; }
              if (this.enableBrush) { this.enableBrush = false; }
          ...
          if (this.tags.length === 0) {
            ...
            this.networks.map((n: Network) => {
              ...
              _.tags.map((t: { text: string }) => {
                if (t.text === n.text) { isMatched = true; }
              });
